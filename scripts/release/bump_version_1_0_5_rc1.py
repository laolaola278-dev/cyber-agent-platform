"""Bump all CAP version carriers from 1.0.4 -> 1.0.5-rc1.

Idempotent: re-running is safe (a carrier already at 1.0.5-rc1 is skipped).

Why a new RC rather than inheriting the v1.0.4 GA certification: the 1.0.5
development line carries multiple runtime-affecting changes classified by
``scripts/release/classify_diff.py`` (fail-closed by design):

    python scripts/release/classify_diff.py 87d2409 abed26f
    -> runtime_affecting=true, RECERTIFICATION_REQUIRED

Runtime-affecting changes on this line:
  * ``backend/app/worker/runtime.py`` -- cooperative lease-heartbeat teardown
    (0708012): the bare ``heartbeat_task.cancel()`` in ``execute``'s finally
    could land while a TTL renewal UPDATE was in flight; the abandoned
    ...
    release UPDATE (product race, triaged in
    docs/quality/flake-triage-34012500372-pregate-d.md).
  * ``frontend/src/**`` -- route-level code-splitting (3ad23e5): 15 pages via
    React.lazy, 1213 kB monolith eliminated, first-paint gzip 437->243 kB.
  * ``frontend/Dockerfile`` -- libuuid CVE remediation (000dee1): ``apk
    upgrade --no-cache libuuid`` clears 7 HIGH CVEs from the util-linux
    2.42.1-r0 base-layer drift.
  * ``scripts/quality/assert_coverage_matrix.py`` -- CI governance hardening
    (f82b8cb): stdlib-only assertion + backend job fail-fast.

The v1.0.4 GA certification therefore CANNOT be inherited. This line must earn
its own certification at the 1.0.5-rc1 anchor, after which a pure
version-metadata commit promotes rc1 -> 1.0.5 (INHERITED).
...
so all carriers must move together. This script makes targeted, surgical edits:
  ...
  * package-lock.json: ONLY the root .version and packages[''].version are
    ...
    be touched.
  * uv.lock: root package version uses PEP 440 form (1.0.5rc1, no hyphen).
  * Chart.yaml: also flips artifacthub prerelease annotation false -> true
    (the 1.0.4 promote commit set it back to "false").
  * test_phase_23_release_candidate.py: RC_VERSION constant tracks the release,
    and the release-notes list gains docs/releases/v1.0.5-rc1.md. That test
    asserts the notes FILE EXISTS, so docs/releases/v1.0.5-rc1.md must be
...
OLD = "1.0.4"
NEW = "1.0.5-rc1"          # SemVer / canonical form
NEW_PEP440 = "1.0.5rc1"     # PEP 440 form (uv.lock)
...
if NEW not in lock_text:
    root_pat = re.compile(r'^  "version": "1\.0\.4",$', re.MULTILINE)
    ...
    changed.append(
        "frontend/package-lock.json: root + packages[''] -> 1.0.5-rc1 (delayed-stream untouched)"
    )
else:
...
#     not touch any dependency that happens to be version 1.0.4.
