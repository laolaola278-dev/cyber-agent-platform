"""Bump all CAP version carriers from 1.0.0 -> 1.0.1-rc1.

Idempotent: re-running is safe (a carrier already at 1.0.1-rc1 is skipped).
...
so all carriers must move together. This script makes targeted, surgical edits:
  ...
  * package-lock.json: ONLY the root .version and packages[''].version are
    ...
    be touched.
  * uv.lock: root package version uses PEP 440 form (1.0.1rc1, no hyphen).
...
OLD = "1.0.0"
NEW = "1.0.1-rc1"          # SemVer / canonical form
NEW_PEP440 = "1.0.1rc1"     # PEP 440 form (uv.lock)
...
# 4. frontend package.json (root version only; deps use ranges, not exact 1.0.0)
...
#    Both are already-quoted string values; delayed-stream's "1.0.0" at
#    node_modules/delayed-stream (6-space indent) must be left alone.
lock = ROOT / "frontend/package-lock.json"
lock_text = lock.read_text("utf-8")
if NEW not in lock_text:
    # root: exactly '  "version": "1.0.0",' (2-space indent, top-level)
    root_pat = re.compile(r'^  "version": "1\.0\.0",$', re.MULTILINE)
    assert len(root_pat.findall(lock_text)) == 1, "root .version not uniquely found"
    lock_text = root_pat.sub(f'  "version": "{NEW}",', lock_text)
    # packages['']: version line that directly follows the "" block's name line
    pkg_pat = re.compile(
        r'("": \{\n      "name": "cyber-agent-platform-frontend",\n      "version": ")1\.0\.0(",)'
    )
    assert len(pkg_pat.findall(lock_text)) == 1, "packages[''].version not uniquely found"
    lock_text = pkg_pat.sub(
        lambda m: f'{m.group(1)}{NEW}{m.group(2)}', lock_text
    )
    lock.write_text(lock_text, "utf-8")
    changed.append("frontend/package-lock.json: root + packages[''] -> 1.0.1-rc1 (delayed-stream untouched)")
else:
...
#     not touch any dependency that happens to be version 1.0.0.
...
# 13. test_phase_23 RC_VERSION constant
edit(
