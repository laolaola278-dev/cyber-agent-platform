"""Promote all CAP version carriers from 1.0.5-rc1 -> 1.0.5 (GA).

This is the pure version-metadata commit that follows the v1.0.5-rc1
certification anchor (901013a). It carries NO code change, so
``scripts/release/classify_diff.py`` must report:

    release_metadata_only = true
    INHERITED            (the rc1 certification carries over)

Idempotent: re-running is safe (a carrier already at 1.0.5 is skipped).

Differences vs the rc1 bump:
  * Chart.yaml flips the artifacthub prerelease annotation back to "false"
    (rc1 set it to "true").
  * test_phase_23 gains docs/releases/v1.0.5.md in the release-notes list.
  * uv.lock root package goes from the PEP 440 form 1.0.5rc1 -> 1.0.5.
...
like "1.0.5" at 6-space indentation, so a substring guard such as
...
See bump_version_1_0_3.py / bump_version_1_0_4.py for why
...
OLD = "1.0.5-rc1"
NEW = "1.0.5"              # SemVer / canonical form
OLD_PEP440 = "1.0.5rc1"     # PEP 440 form currently in uv.lock
NEW_PEP440 = "1.0.5"
...
# entry, never a dependency that happens to be version 1.0.5.
