"""Bump all CAP version carriers from 1.0.2 -> 1.0.3-rc1.

Idempotent: re-running is safe (a carrier already at 1.0.3-rc1 is skipped).

Why a new RC rather than inheriting the v1.0.2 GA certification: the frontend
console refactor (8a27719, split into per-domain pages with a shared http
client + usePageList hook) plus the CI blind-spot governance doc (8718481) and
the CI lint fix (6a16799) touch ``frontend/src/**``, which
``scripts/release/classify_diff.py`` classifies as ``production_runtime``:

    python scripts/release/classify_diff.py 9aad312 6a16799
    -> runtime_affecting=true, RECERTIFICATION_REQUIRED

The v1.0.2 GA certification therefore CANNOT be inherited. The frontend
refactor must earn its own certification at a fresh rc anchor, after which a
pure version-metadata commit promotes rc1 -> 1.0.3 (INHERITED).
...
so all carriers must move together. This script makes targeted, surgical edits:
  ...
  * package-lock.json: ONLY the root .version and packages[''].version are
    ...
    be touched.
  * uv.lock: root package version uses PEP 440 form (1.0.3rc1, no hyphen).
  * Chart.yaml: also flips artifacthub prerelease annotation false -> true
    (the 1.0.2 promote commit set it back to "false").
  * test_phase_23_release_candidate.py: RC_VERSION constant tracks the release,
    and the release-notes list gains docs/releases/v1.0.3-rc1.md. That test
    asserts the notes FILE EXISTS, so docs/releases/v1.0.3-rc1.md must be
...
OLD = "1.0.2"
NEW = "1.0.3-rc1"          # SemVer / canonical form
NEW_PEP440 = "1.0.3rc1"     # PEP 440 form (uv.lock)
...
if NEW not in lock_text:
    root_pat = re.compile(r'^  "version": "1\.0\.2",$', re.MULTILINE)
    ...
    changed.append(
        "frontend/package-lock.json: root + packages[''] -> 1.0.3-rc1 (delayed-stream untouched)"
    )
else:
...
#     not touch any dependency that happens to be version 1.0.2.
