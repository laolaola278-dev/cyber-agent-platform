"""Promote all CAP version carriers from 1.0.3-rc1 -> 1.0.3 (GA).

This is the pure version-metadata commit that follows the v1.0.3-rc1
certification anchor (06b74b8). It carries NO code change, so
``scripts/release/classify_diff.py`` must report:

    release_metadata_only = true
    INHERITED            (the rc1 certification carries over)

Idempotent: re-running is safe (a carrier already at 1.0.3 is skipped).

Differences vs the rc1 bump:
  * Chart.yaml flips the artifacthub prerelease annotation back to "false"
    (rc1 set it to "true").
  * test_phase_23 gains docs/releases/v1.0.3.md in the release-notes list.
  * uv.lock root package goes from the PEP 440 form 1.0.3rc1 -> 1.0.3.
...
like "1.0.3" at 6-space indentation, so a substring guard such as
...
See bump_version_1_0_3_rc1.py for why package-lock.json and uv.lock need
targeted edits (delayed-stream / dependency false positives).
"""
...
OLD = "1.0.3-rc1"
NEW = "1.0.3"              # SemVer / canonical form
OLD_PEP440 = "1.0.3rc1"     # PEP 440 form currently in uv.lock
NEW_PEP440 = "1.0.3"
...
# entry, never a dependency that happens to be version 1.0.3.
