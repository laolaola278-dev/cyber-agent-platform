...
function App() {
  ...
  const content: Record<PageKey, React.ReactNode> = {
    ...
    workers: <WorkersPage />,
    approvals: <ApprovalsPage approvals={approvals} loading={loading} error={error} onRetry={() => void refresh()} onChanged={() => void refresh()} />,
    plugins: <PluginsPage plugins={plugins} loading={loading} error={error} onRetry={() => void refresh()} />,
    audit: <AuditPage />,
