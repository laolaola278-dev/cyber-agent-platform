...
async def db(tmp_path) -> tuple:
    ...
    connection across ALL sessions. The claim-loop execution path opens
    its own poll sessions (``async_sessionmaker(service.session.bind)`` in
    worker_path), so operation COMMITs and poll SELECTs interleave on that
    single connection -- the load-sensitive
    ``cannot commit transaction - SQL statements in progress`` /
    ``RUNNING != COMPLETE`` flake seen in CI (run 33946618296; classified
    harness race in docs/quality/cert-rerun-triage-33946618296.md). A
    per-test file DB gives each session its own connection, mirroring the
    production PostgreSQL topology. Same pattern as the 28.2 cancellation
    suite.
    """
...
async def test_observability_fields_durable(session, db, tmp_path, lab) -> None:
    service = await _make_service(session, tmp_path, lab)
    run, _ = await service.create(goal="g", url=f"{lab.origin}/static")
    await session.flush()
    reg = WorkerRegistry(session)
