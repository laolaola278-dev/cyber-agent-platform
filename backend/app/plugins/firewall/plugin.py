"""Mock-only Firewall Response Plugin using the existing Response SDK lifecycle."""
...
class FirewallResponsePlugin:
    """Governed network-layer response backed exclusively by a synthetic provider."""

    name = "firewall-response"
    version = "1.0.0"
    description = "Mock-only declarative firewall rule response plugin"
    capabilities = frozenset({"response.firewall"})
    ...
    def __init__(self, adapter: FirewallAdapter) -> None:
        ...
        self._pending_verification: (
            tuple[FirewallRuleChange, FirewallRollbackAction | None] | None
        ) = None

    async def initialize(self, context: ResponsePluginContext) -> None:
        if context.granted_permissions != self.permissions:
            raise ResponsePolicyViolation("Firewall plugin requires its certified permission set")
        if context.capability != "response.firewall":
            raise ResponsePolicyViolation("Firewall plugin accepts only response.firewall plans")
    ...
    async def plan(
        ...
        return plan.model_copy(
            deep=True,
            update={"parameters": {"rule": rule.model_dump(mode="json")}},
        )

    async def validate(self, plan: ResponsePlanSpec, context: ResponsePluginContext) -> None:
        self._require_context(context)
        if plan.target_capability != "response.firewall":
            raise ResponsePolicyViolation("Firewall plan capability is invalid")
        if plan.incident_id != context.incident_id or tuple(plan.asset_ids) != context.asset_ids:
            raise ResponsePolicyViolation("Firewall plan scope does not match immutable context")
        ...
        if rule.id.casefold().startswith(("system-", "provider-", "builtin-")):
            raise ResponsePolicyViolation("Provider-owned Firewall rules cannot be modified")
    ...
    async def verify(
        ...
        if self._pending_verification is None:
            raise ResponseExecutionError("Firewall verification has no pending provider operation")
    ...
    async def rollback(
        ...
        if not context.rollback_token or not self._valid_rollback_token(context):
            raise ResponsePolicyViolation("Validated Firewall rollback token is required")
        started = monotonic()
        rule = self._adapter.parse_rule(plan.parameters)
        ...
        change = await self._adapter.rollback(
            rule_id=rule.id,
            action=action,
            original_rule=rule if action is FirewallRollbackAction.RESTORE else None,
        )
    ...
    def _result(
        ...
        *,
        ...
        return ResponseResult(
            ...
            message=f"Mock Firewall rule operation {change.operation} completed",
    ...
    def _rollback_token(self, context: ResponsePluginContext, rule: FirewallRule) -> str:
        material = (
            f"{context.response_plan_id}:{context.incident_id}:{rule.id}:"
            f"{rule.version}:{rule.checksum}"
        )
        return f"firewall-rb:{hashlib.sha256(material.encode('utf-8')).hexdigest()}"
    ...
    def _require_context(self, context: ResponsePluginContext) -> None:
        if self._context != context:
            raise ResponseExecutionError("Firewall plugin was not initialized for this context")
