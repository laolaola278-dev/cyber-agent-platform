...
_PERMISSION_SPECS = {
    ...
    "acquisition.read": ("acquisition", "read", "View acquisition runs and evidence"),
    "acquisition.execute": ("acquisition", "execute", "Start, resume or cancel acquisition runs"),
    "agent.read": ("agent", "read", "View agents, evaluations and investigations"),
    "agent.execute": ("agent", "execute", "Run agent triage, investigation and analysis"),
    "task.read": ("task", "read", "View tasks and their executions"),
    "task.write": ("task", "write", "Create tasks"),
    "workflow.read": ("workflow", "read", "View workflow definitions and runs"),
    "workflow.execute": ("workflow", "execute", "Plan, run, resume or cancel a workflow"),
    "registry.read": ("registry", "read", "View the agent/tool registry"),
    "registry.write": ("registry", "write", "Register or retire registry entries"),
    "capability.read": ("capability", "read", "View advertised capabilities"),
    "runtime.read": ("runtime", "read", "View runtime status"),
    "runtime.write": ("runtime", "write", "Start or stop runtime instances"),
...
_SOC_PERMISSIONS = frozenset(
    {
        ...
        "playbook.read",
        "playbook.execute",
        # The analyst plane reads and drives acquisitions, tasks, workflows, agents
        # and investigations. Those paths fell through to `platform.manage` until
        # the console-routing gate showed the cost: an operator who wanted the
        # Acquisitions page had to be granted the whole platform.
        "acquisition.read",
        "acquisition.execute",
        "agent.read",
        "agent.execute",
...
_RESPONDER_PERMISSIONS = frozenset(
    {
        ...
        "response.execute",
        "response.rollback",
        # A responder sees the operational plane it acts on but does not drive
        # acquisitions, tasks or the registry.
        "acquisition.read",
        "agent.read",
        "capability.read",
        "registry.read",
