...
gate and its 17 tests, F-22/F-23's GA-generator fixes and their 9 executed tests, the corrected
comments, and the unit job's own JUnit), on which all five CI jobs are green (run `35453419340`) and
`classify_diff c52dcb9 2e4ecaf` reports **INHERITED** across 13 files (`ci_workflow`, `test_harness`,
`certification_generator`, `docs`, `repo_tooling`). Report-text commits after it are
documentation only, and the gate re-derives that judgement at tag time rather than trusting this
sentence. Machine-readable proofs: `outputs/cert-becbad4/diff-*.json`, `outputs/cert-c52dcb9/`,
`outputs/cert-e4b4e86/` (including the publication gate's own evidence JSON for this tip) and
`outputs/cert-ci-2e4ecaf/`.
...
| `c52dcb9 → 2e4ecaf` (last CI-verified tip: F-21's gate, F-22/F-23's generator fixes, their tests, the comment corrections, CI's JUnit) | **INHERITED** | none: 5 `ci_workflow`, 4 `test_harness`, 2 `docs`, 1 `certification_generator`, 1 `repo_tooling` (13 files) — `outputs/cert-e4b4e86/diff-c52dcb9-to-2e4ecaf.json` |
...
re-executed at the CI-verified tip `2e4ecaf` with the same result: **1042 files, 0 findings**
(`outputs/cert-ci-2e4ecaf/secret-scan.json`).
...
container are in no denominator anywhere. Re-run against the final tip's own uploaded
`coverage.xml` it is **91.8%** with the same three modules below 80% (`database` 75.9%,
`assessment` 76.0%, `incident` 77.4%) — the same shape, measured rather than carried forward
(`outputs/cert-ci-2e4ecaf/coverage-low-modules.json`).
`outputs/cert-c52dcb9/coverage-low-modules.json` — generated by
...
**The same audit generated from CI's own run, not from its log.** The unit job now writes
`junit-backend.xml` and uploads it beside `coverage.xml` in `backend-evidence`
(`test_quality_gate_parity.py` fails if a report a job generates has no uploader), so
`audit_junit.py` can be pointed at the CI run itself. Run `35453419340` at the CI-verified tip
`2e4ecaf` (`outputs/cert-ci-2e4ecaf/`; the commit before it, `953c43c`, in
`outputs/cert-ci-953c43c/`): **1546 collected, 1413 run, 133 skipped, 0 failed**, by family
kubernetes 56, postgres 35, other 14, object store 12, OCI runtime 9, browser 7, with 8
security-relevant tests listed by name. The 147/133 gap reconciles exactly once the two files are
diffed rather than compared by headline: 15 of the local skips belong to the five `--ignore`d
Linux/container suites, which CI does not collect at all (absent from its JUnit, and executed by
§13's certification layer instead), leaving 132 shared; CI's 133 is those plus one skip that can
only exist there — `test_release_publication_gate.py::test_the_actions_api_paths_the_gate_uses_exist`,
which skips when the job token cannot answer the Actions API, exactly as §23 F-21 requires of a test
that must never decide a release's CI verdict. Same families, same homes, one attributable delta on
each side.
...
- F-21 **The release pipeline had no certification gate, and a comment said it did.**
  ...
  **Re-run after that GA round went green, the same code returned PASS** for the final tip
  `2e4ecaf`: Linux `35439343387`, K8s `35439344924`, GA `35445391334` and the soak `35439341789`
  all resolved to `c52dcb9` at distance 19 and classified INHERITED, `failures: []`
  (`outputs/cert-e4b4e86/gate-at-2e4ecaf.json`). Both verdicts came from the same execution of
...
- CI's five jobs green on the certified SHA (`35439324779`) and again on the final tip
  (`35453419340` at `2e4ecaf`: backend unit **1546 collected / 1413 run / 133 skipped / 0 failed**
  at 91.8% from the JUnit and coverage files that run uploaded, plus frontend
  lint/vitest/typecheck/build, migration on PostgreSQL 16, packaging, and image-and-security with
  Trivy and pinned actions);
- local backend suite **1389 passed / 147 skipped / 1 deselected / 0 failed** at 91.9% (the same
  commit's CI unit job: 1386 passed, 0 failed, 91.72%, read from its log — the job writes JUnit only
  from the tip onward), with the skip and coverage tables generated by
  `scripts/quality/audit_junit.py` rather than transcribed (§3, §21, §22);
- credential scan 1040 tracked files, 0 findings, at the certified SHA and 1042 / 0 at the
  CI-verified tip, with planted-credential and reference-shaped controls both directions (§20);
