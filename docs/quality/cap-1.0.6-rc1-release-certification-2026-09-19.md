...
(40/40, run `35445391334`) all ran green on. Last CI-verified tip: `953c43c` (F-21's publication
gate, its 17 tests, the corrected comments and the unit job's own JUnit), on which all five CI jobs
are green (run `35450100427`) and `classify_diff c52dcb9 953c43c` reports **INHERITED** across 10
files (`ci_workflow`, `test_harness`, `docs`, `repo_tooling`). Report-text commits after it are
...
| `c52dcb9 → 953c43c` (last CI-verified tip: F-21's gate, its tests, the comment corrections, CI's JUnit) | **INHERITED** | none: 4 `ci_workflow`, 3 `test_harness`, 2 `docs`, 1 `repo_tooling` (10 files) — `outputs/cert-e4b4e86/diff-c52dcb9-to-953c43c.json` |
...
re-executed at the final tip `953c43c` with the same result: **1041 files, 0 findings**
(`outputs/cert-ci-953c43c/secret-scan.json`).
...
`coverage.xml` it is **91.7%** with the same three modules below 80% — the same shape, measured
rather than carried forward (`outputs/cert-ci-953c43c/coverage-low-modules.json`).
...
**The same audit generated from CI's own run, not from its log.** The unit job writes
`junit-backend.xml` and uploads it with `coverage.xml` in `backend-evidence` (run `35450100427` at
the final tip `953c43c`; `test_quality_gate_parity.py` now fails if a report a job generates has no
uploader), so `audit_junit.py` can be pointed at the CI run directly
(`outputs/cert-ci-953c43c/`): **1537 collected, 1404 run, 133 skipped, 0 failed**, by family
...
- F-21 **The release pipeline had no certification gate, and a comment said it did.**
  ...
  **Re-run after that GA round went green, the same code returned PASS** for the final tip
  `953c43c`: Linux `35439343387`, K8s `35439344924`, GA `35445391334` and the soak `35439341789`
  all resolved to `c52dcb9` at distance 17 and classified INHERITED, `failures: []`
  (`outputs/cert-e4b4e86/gate-at-953c43c.json`). Both verdicts came from the same execution of
...
- CI's five jobs green on the certified SHA (`35439324779`) and again on the final tip
  (`35450100427` at `953c43c`: backend unit **1537 collected / 1404 run / 133 skipped / 0 failed**
  at 91.7% from the JUnit and coverage files that run uploaded, plus frontend
...
- credential scan 1040 tracked files, 0 findings, at the certified SHA and 1041 / 0 at the final
  tip, with planted-credential and reference-shaped controls both directions (§20);
